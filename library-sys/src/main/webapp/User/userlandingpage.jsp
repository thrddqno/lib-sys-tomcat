			            ...
			            <div class="col-md-3 py-2">
			            	<div class="card" style="width: 23rem;">
			            		...
			            		<div class="card-body overflow-hidden">
			            			<a class="card-title primary-text" href="<%=request.getContextPath()%>/user/viewbook?id=${book.bookId}">${book.title}</a>
			            			<p class="card-text author second-text">${book.author}</p>
			            			<p class="card-text card-desc second-text">${book.description}</p>
			            		</div>
			            		<div class="d-flex justify-content-start">
			            			<p class="category second-text">
								        ${book.categories}
								    </p>
			            		</div>
			            	</div>
	        	...
	        	<ul class="pagination pagination-sm justify-content-center">
	        		<c:forEach var="i" begin="1" end="${totalPages}">
		        		<li class="page-item ${currentPage eq i ? 'active' : ''}">
		        			<a class="page-link" href="<%=request.getContextPath()%>/user/books?page=${i}">${i}</a>
		        		</li>
		        	</c:forEach>
	        	</ul>
	        </nav>
